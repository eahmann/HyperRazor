...
**Date:** 2026-03-09  
**Status:** Revised draft; design-direction document, not final API spec  
**Supersedes:** `docs/validation-framework-dx-refactor-spec-v2.md`  
**Reviewed input:** `docs/validation-framework-spec-feedback.md`, `docs/validation-framework-dx-refactor-spec-v2-feedback.md`
...
This revision carries forward the earlier feedback already adopted in v2 and additionally reviews every major point in `docs/validation-framework-dx-refactor-spec-v2-feedback.md`.

1. Choose one primary authoring shape. Adopted.
   `HrzField` plus child controls is now the documented default; standalone `HrzInput* For=...` is deferred.
2. Explain the non-interactive form and field runtime more concretely. Adopted.
   The spec now defines a request-scoped `HrzForm` context consumed by `HrzField`.
3. Tighten the first-pass checkbox story. Adopted with explicit semantics.
   `HrzCheckbox` remains first-pass only as a defined special case, with `bool?` and tri-state behavior deferred.
4. Give the descriptor model a minimum concrete shape. Adopted.
   The spec now includes a minimum `HrzValidationDescriptor` and `HrzFieldDescriptor` shape.
5. Clarify guaranteed local-client support limits. Adopted.
   Only rules emitted into compatible browser metadata are guaranteed local-client support in v1.
6. Define summary merge behavior explicitly. Adopted.
   The spec now states the default rules for submit, live, and client-local summary behavior.
7. Tighten the invalid-helper contract. Adopted.
   MVC and Minimal API helpers must support local, backend-mapped, and merged invalid state.
8. Sharpen descriptor wording so state and descriptors stay distinct. Adopted.
   The descriptor layer now explicitly owns field identity, validation metadata, and DOM emission rather than request-specific validation state.
