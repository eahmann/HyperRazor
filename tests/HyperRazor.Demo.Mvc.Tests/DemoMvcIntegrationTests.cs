...
public class DemoMvcIntegrationTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    [Fact]
    public async Task LiveValidation_WithMissingDisplayNameDependency_ReturnsDependentDisplayNameError()
    {
        using var client = CreateClient();
        var antiforgeryToken = await GetAntiforgeryTokenAsync(client);
        using var request = new HttpRequestMessage(HttpMethod.Post, "/validation/live");
    ...
    [Fact]
    public async Task LiveValidation_DisplayNameScope_PreservesEmailSummaryWhenEmailRemainsBackendInvalid()
    {
        using var client = CreateClient();
        var antiforgeryToken = await GetAntiforgeryTokenAsync(client);
        using var request = new HttpRequestMessage(HttpMethod.Post, "/validation/live");
        ...
        Assert.Contains("Email already exists in the upstream directory.", body, StringComparison.Ordinal);
        Assert.Contains("id=\"validation-minimal-proxy-summary\"", body, StringComparison.Ordinal);
        Assert.Contains("Backend would reject this invite on submit.", body, StringComparison.Ordinal);
    }
